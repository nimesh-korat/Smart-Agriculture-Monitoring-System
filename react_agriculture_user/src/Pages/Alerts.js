    ...
    const fetchData = async () => {
        try {
            const response = await axios.post(
                "http://localhost:8000/api/user/getAlerts"
            );
                        ...
                        <div className="container py-5">
                            <div className="row g-5">
                                ...
                                {data.length > 0 ? data.map((sensor, index) => (
                                    <div
                                        className="col-4 wow fadeIn justify-content-center"
                                        key={index}
                                        data-wow-delay="0.3s"
                                    >
                                        <div
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                {sensor.sensorName === "Flame Sensor" && (
                                                    <i className="fad fa-fire-smoke fa-3x text-primary" />
                                                )}
                                                {sensor.sensorName === "Smoke Sensor" && (
                                                    <i className="fad fa-heat fa-3x text-primary" />
                                                )}
                                                {sensor.sensorName === "Water Sensor" && (
                                                    <i className="fa fa-dewpoint fa-3x text-primary" />
                                                )}
